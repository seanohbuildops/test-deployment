const vision = require('@google-cloud/vision');
const url = require('url');

const extractData = async (imageUrl, tenantId) => {

  // dissect url and validate host and path
  const parsedUrl = url.parse(imageUrl)

  if(parsedUrl.protocol !== 'https:'){
    console.log('error observed:' + parsedUrl.protocol);
    throw new Error('Invalid Protocol');
  }

  if(parsedUrl.host !== `${process.env.TENANT_CONTENT_BUCKET_NAME}.s3.amazonaws.com`){
    console.log('error observed:' + parsedUrl.host);
    throw new Error('Invalid Host');
  }

  if(!parsedUrl.pathname.startsWith(`/public/${tenantId}/`)){
    console.log('error observed:' + parsedUrl.host);
    throw new Error('Invalid Pathname');
  }

  const credentials = {
    client_email: "aryan-986@lyrics-171420.iam.gserviceaccount.com",
    private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  };

  const client = new vision.ImageAnnotatorClient({ credentials: credentials });
  const imageRequest = { image: { source: { imageUri: imageUrl } } };
  const [result] = await client.textDetection(imageRequest);
  const detections = result.textAnnotations;
  const words = detections.map(x => x.description);
  return {
    data: words,
  };
};

module.exports.handler = async event => extractData(event.imageUrl, event.tenantId);
